...
def signup(request):
    if request.method == 'POST':
        ...
        try:
            tempusername = User.objects.get(username=tempusername)
...
def post_view(request, post_id):
    ...
    current_user_id = request.session.get('logged_user_id')
    #
    comments = Comment.objects.filter(post = current_post).order_by('-created_on')
    try:
        current_user = User.objects.get(id=current_user_id)
            ...
            if regex.search(userinput) == None:
                ...
                try:
                    ...
                    for temppost in tempPosts:
                        if userinput in temppost.postTitle:
                            resultPosts.add(temppost)                         
                        elif userinput in temppost.postBody:
                            resultPosts.add(temppost)                                                  
                    #end for
                    context = {
                        'user': user,
                ...
                #input has #, check tags
                tags = filter(None, userinput.split('#'))
